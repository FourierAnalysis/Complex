...
CP.yAxis  = SD.lineMaker   ({x1:0, x2:0, y1:-CP.size, y2:CP.size, style:'->', arrowSize: CP.size/10});
CP.xAxis  = SD.lineMaker   ({y1:0, y2:0, x1:-CP.size, x2:CP.size, style:'->', arrowSize: CP.size/10});

CP.point  = SD.circleMaker ({ x:2,  y:3, r:0.05, svgAttributes: {fill: 'white'}});
CP.vector = SD.lineMaker   ({x1:0, y1:0, x2:CP.point.x, y2:CP.point.y, style: '-->', arrowSize: CP.size/15, color:'red'});
CP.xProj  = SD.lineMaker   ({x1:CP.point.x, y1:CP.point.y, x2:CP.point.x, y2:0, style:'--', color:'blue'});
CP.yProj  = SD.lineMaker   ({x1:CP.point.x, y1:CP.point.y, x2:0, y2:CP.point.y, style:'--', color:'blue'});
